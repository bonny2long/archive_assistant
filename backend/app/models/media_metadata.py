...
class UniversalIngestionReviewAction(Base):
    __tablename__ = "universal_ingestion_review_actions"
    ...
    target_media_class: Mapped[str | None] = mapped_column(String(80), nullable=True)
    target_candidate_id: Mapped[int | None] = mapped_column(ForeignKey("media_identity_candidates.id"), nullable=True, index=True)
    override_title: Mapped[str | None] = mapped_column(Text, nullable=True)
    override_primary_creator: Mapped[str | None] = mapped_column(Text, nullable=True)
    override_year: Mapped[str | None] = mapped_column(String(20), nullable=True)
    override_series: Mapped[str | None] = mapped_column(Text, nullable=True)
    override_series_index: Mapped[str | None] = mapped_column(String(50), nullable=True)
    override_release_type: Mapped[str | None] = mapped_column(String(80), nullable=True)
    override_genre_family: Mapped[str | None] = mapped_column(Text, nullable=True)
    override_destination_root: Mapped[str | None] = mapped_column(Text, nullable=True)
    decision_status: Mapped[str] = mapped_column(String(30), default="active", index=True)
    reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    note: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now_utc)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now_utc)
    applied_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    created_by: Mapped[str | None] = mapped_column(String(100), default="local_user", nullable=True)